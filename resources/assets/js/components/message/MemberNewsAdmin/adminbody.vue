        ...
        <form name="form" method="post" action="">
            <table width="100%" border="0" cellspacing="0" cellpadding="5" class="tableLists">
                ...
                <tr v-for="(NewAdminList,index) in NewAdminLists" :key="NewAdminLists.id">
                    ...
                    <td width="6%" align="center">{{NewAdminList.categoryPid_name}}</td>
                    ...
                    <td align="center" @click="shoulu(NewAdminList.id,index)" v-if="NewAdminList.news_id ==0 "><a href="javascript:void(0)">未收录 </a></td>
                    <td align="center"  v-else>已收录</td>
                    <td align="center" @click="deleteTodo(NewAdminList,index)">删除</td>
                    <!--<td width="7%" align="center"><router-link :to="'/admin/message/getnewsEditMemberNews/'+NewAdminList.id" tag="a">编辑</router-link></td>-->
...
<script>
    ...
    mounted() {
        ...
        axios.get('/admin/message/MemberNewsAdminList').then(response=>{
            if(response.data == 0){
        ...
        methods: {
            deleteTodo(NewAdminList,index){
                ...
                if (r==true)
                {
                axios.post('/admin/message/deleteMemberNewsAdminList', {
                    id:NewAdminList.id,
                }).then(response=> {
                    if(response.data == 0){
                        this.$router.push({path:'/admin/role_error/'});
            ...
            shoulu(NewAdminList,index) {
                axios.post('/admin/message/shoulu', {
                    id:NewAdminList,
                }).then(response=>{
                    if(response.data == 0){
                        this.$router.push({path:'/admin/role_error/'});
                    }else {
                        this.$router.push({path:'/admin/message/newsEdit/'+response.data.id});
                    }
                }).catch(function (error) {
                    alert(error);
                });
