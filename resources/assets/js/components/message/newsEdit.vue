            ...
            <form v-on:submit.prevent="updateNewAdmin(NewAdmin)">
                <table width="100%" border="0" cellspacing="0" cellpadding="5" class="tableLists">
                    ...
                    <tr>
                        ...
                        <td rowspan="5" align="center"><div style="width:200px; height:136px; background:#CCC;">

                            <img v-bind:src="NewAdmin.image" style="width:200px; height:136px; background:#CCC;"/>
                        </div></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><input type="radio" name="import_type" value="1" v-model="NewAdmin.is_origin" v-bind:value="1">
                            否
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="radio" name="radiobutton" v-model="NewAdmin.publics" v-bind:value="0" >
                            所有人            </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="3"><input type="radio" name="publics" v-model="NewAdmin.publics" v-bind:value="1">
                            仅所有会员
                            <select  v-model="NewAdmin.publics_level" >
                                <option selected value="0">所有会员</option>
                                <option v-for="(item ,index) in levels " v-bind:value="item.id">{{item.level_name}}</option>
                            </select>
                            <input name="checkbox42" type="checkbox" v-model="NewAdmin.is_auth" v-bind:value="1" >
                            仅认证会员可浏览 （<a href="#" @click="Users_role()">设置黑白名单</a>）</td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td align="right">&nbsp;</td>
...
<script>
    ...
    import stateTag from './AdminstateTag'
    import namelist from './NameList'
    export default {
        name: "updateNewAdminList",
        components:{
            topline,stateTag,namelist
        },
        methods:{
            ...
            updateNewAdmin:function (NewAdmin) {
                    ...
                    axios.post('/admin/message/updateNewAdmin', {
                        id:NewAdmin.id,
                        Users:this.Users,
        ...
        mounted() {
            axios.get('/admin/message/getnewsEdit/'+this.$route.params.id).then(response=>{
                if(response.data == 0){
            ...
            axios.get('/admin/message/getAdminTabNewsByNews/'+this.$route.params.id).then(response=>{
